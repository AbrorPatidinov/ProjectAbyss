...
void generate_native_code(const char *out_filename, int enable_profiler)
{
    ...
    fprintf(f, "#define C_RESET \"\\033[0m\"\n#define C_BOLD \"\\033[1m\"\n#define "
    ...
    if (enable_profiler)
    {
        ...
        fprintf(f, "#define ALLOC_BUCKETS 65536\n");
        fprintf(f, "AllocInfo *alloc_table[ALLOC_BUCKETS] = {NULL};\n");
        fprintf(f, "int stack_alloc_count = 0;\n\n");

        fprintf(f, "void track_alloc(void *ptr, uint32_t sid, uint32_t size, int "
                   "is_stack, char *comment, size_t ip, size_t fp) {\n");
        fprintf(f, "  uint32_t idx = ((uintptr_t)ptr >> 4) & 0xFFFF;\n");
        fprintf(f, "  AllocInfo *node = malloc(sizeof(AllocInfo)); node->ptr = ptr; "
                   ...
                   "= 0; node->next = alloc_table[idx]; alloc_table[idx] = node;\n"
                   "  if (is_stack) stack_alloc_count++;\n}\n");

        fprintf(f, "void untrack_alloc(void *ptr, size_t ip) {\n"
                   "  uint32_t idx = ((uintptr_t)ptr >> 4) & 0xFFFF;\n"
                   "  AllocInfo *curr = alloc_table[idx]; while(curr) { if(curr->ptr == ptr && "
                   "!curr->is_freed) { curr->is_freed = 1; curr->free_ip = ip; "
                   "return; } curr = curr->next; }\n}\n");

        fprintf(f, "void free_stack_allocs(size_t current_fp, size_t ip) {\n"
                   "  if (stack_alloc_count == 0) return;\n"
                   "  for(int i=0; i<ALLOC_BUCKETS; i++) { AllocInfo "
                   "*curr = alloc_table[i]; while(curr) { if(curr->is_stack && "
                   "!curr->is_freed && curr->alloc_fp >= current_fp) { curr->is_freed = "
                   "1; curr->free_ip = ip; free(curr->ptr); stack_alloc_count--; "
                   "} curr = curr->next; } }\n}\n");

        fprintf(f, "void abyss_eye() {\n");
        fprintf(f, "  uint32_t as=0, ah=0, tf=0, ta=0; int ac=0; "
                   "for(int i=0; i<ALLOC_BUCKETS; i++) { AllocInfo *c = alloc_table[i]; "
                   "while(c){ ta++; if(c->is_freed) tf+=c->size; else { ac++; "
                   "if(c->is_stack) as+=c->size; else ah+=c->size; } c=c->next; } }\n");
        fprintf(f, "  printf(\"\\n\" C_BOLD C_CYAN \"  "
                   "╭───────────────────────────────────────────────────────────────────"
                   "───────────────────────────────────────────╮\\n\" C_RESET);\n");
        fprintf(f, "  printf(C_BOLD C_CYAN \"  │\" C_RESET C_BOLD \"  📊   S Y S T E M  "
                   " R E S O U R C E S                                                  "
                   "                     \" C_BOLD C_CYAN \"│\\n\" C_RESET);\n");
        fprintf(f, "  printf(C_BOLD C_CYAN \"  "
                   "├───────────────────────────────────────────────────────────────────"
                   "───────────────────────────────────────────┤\\n\" C_RESET);\n");
    ...
    {
        // --- NO PROFILER: empty stubs, zero overhead ---
        fprintf(f, "static inline void track_alloc(void *p, uint32_t s, uint32_t sz, "
                   "int st, char *c, size_t i, size_t f) { (void)p;(void)s;(void)sz;"
                   "(void)st;(void)c;(void)i;(void)f; }\n");
        fprintf(f, "static inline void untrack_alloc(void *p, size_t i) { (void)p;(void)i; }\n");
        fprintf(f,
                "static inline void free_stack_allocs(size_t f, size_t i) { (void)f;(void)i; }\n");
        fprintf(f, "void abyss_eye() { printf(\"[Abyss Eye disabled in fast native mode. "
                   "Use --native --eye to enable.]\\n\"); }\n\n");
    }
    ...
    while (ip < code_sz)
    {
        ...
        if (op == OP_CONST_INT || op == OP_CONST_STR || op == OP_JMP || op == OP_JZ ||
            op == OP_TRY || op == OP_NATIVE || op == OP_TAG_ALLOC)
            ip += 4;
        else if (op == OP_CONST_FLOAT)
            ip += 8;
        else if (op == OP_CALL)
            ip += 5;
        else if (op == OP_ALLOC_STRUCT || op == OP_ALLOC_ARRAY || op == OP_ALLOC_STACK)
            ip += 8;
        else if (op == OP_GET_GLOBAL || op == OP_SET_GLOBAL || op == OP_GET_LOCAL ||
                 op == OP_SET_LOCAL || op == OP_RET || op == OP_GET_FIELD || op == OP_SET_FIELD ||
                 op == OP_PRINT_FMT || op == OP_CALL_DYN_BOT)
            ip += 1;
    }
    ...
    while (ip < code_sz)
    {
        ...
        case OP_ALLOC_STRUCT:
        {
            ...
            if (cidx == 0xFFFFFFFF)
            {
                fprintf(f,
                        ...
                        "track_alloc(p, %u, sz, 0, NULL, %zu, fp); stack[sp++].p = p; }\n",
                        sid, sid, ip);
            }
            else
            {
                fprintf(f,
                        "  { uint32_t sz = structs[%u].size * 8; int64_t *p = calloc(1, sz); "
                        "track_alloc(p, %u, sz, 0, strs[%u], %zu, fp); stack[sp++].p = p; }\n",
                        sid, sid, cidx, ip);
            }
            break;
        }
        case OP_ALLOC_ARRAY:
        {
            ...
            {
                fprintf(f,
                        "  { int64_t c = stack[--sp].i; uint32_t sz = c * %u; int64_t "
                        "*p = calloc(1, sz); track_alloc(p, 0xFFFFFFFF, sz, 0, "
                        "\"Array Alloc\", %zu, fp); stack[sp++].p = p; }\n",
                        elem_sz, ip);
            }
            else
            {
        ...
        case OP_ALLOC_STACK:
        {
            ...
            if (cidx == 0xFFFFFFFF)
            {
                fprintf(f,
                        "  { uint32_t sz = structs[%u].size * 8; int64_t *p = calloc(1, sz); "
                        "track_alloc(p, %u, sz, 1, NULL, %zu, fp); stack[sp++].p = p; }\n",
                        sid, sid, ip);
            }
            else
            {
                fprintf(f,
                        "  { uint32_t sz = structs[%u].size * 8; int64_t *p = calloc(1, sz); "
                        "track_alloc(p, %u, sz, 1, strs[%u], %zu, fp); stack[sp++].p = p; }\n",
                        sid, sid, cidx, ip);
            }
            break;
        }
        case OP_FREE:
            fprintf(f, "  { void *p = stack[--sp].p; untrack_alloc(p, %zu); free(p); }\n", ip);
            break;
        ...
        case OP_TRY:
        {
            ...
            ip += 4;
            fprintf(f, "  exception_stack[esp++] = (ExceptionFrame){%u, sp, fp, csp};\n",
                    catch_addr);
            break;
        }
        case OP_END_TRY:
            ...
            break;
        case OP_THROW:
            fprintf(f, "  { Value err_val = stack[--sp]; if (esp == 0) { fprintf(stderr, "
        ...
        case OP_NATIVE:
        {
            ...
            ip += 4;
            if (nid == 0)
                fprintf(f, "  { struct timespec ts; clock_gettime(CLOCK_MONOTONIC, &ts); "
        ...
        case OP_TAG_ALLOC:
        {
            ...
            if (enable_profiler)
            {
                fprintf(f,
                        "  { void *p = stack[sp-1].p; "
                        "uint32_t idx = ((uintptr_t)p >> 4) & 0xFFFF; "
                        "AllocInfo *c = alloc_table[idx]; "
                        "while(c) { if(c->ptr == p && !c->is_freed) { c->tag = strs[%u]; "
                        "break; } c = c->next; } }\n",
                        sidx);
            }
            // When profiling is off: emit nothing (zero overhead)
            break;
        }
        default:
