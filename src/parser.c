    ...
    *struct_id = -1; *array_depth = 0;
    ...
    if (accept(TK_STACK)) {
        ...
        if (accept(TK_COMMA)) {
            ...
            expect(TK_RPAREN);
            if (t != TYPE_STRUCT) fail("stack() expects struct type");
            emit(OP_ALLOC_STACK); emit32(sid);
            *struct_id = sid; return TYPE_STRUCT;
        }
    }
            ...
            if (accept(TK_DOT)) {
                ...
                for (int i=0; i<structs[sid].field_count; i++) {
                    if (!strcmp(structs[sid].fields[i].name, cur.text)) {
                        emit(OP_GET_FIELD);
                        emit(structs[sid].fields[i].offset);
                        DataType new_t = structs[sid].fields[i].type;
                        int new_sid    = structs[sid].fields[i].struct_id;
                        int new_ad     = structs[sid].fields[i].array_depth;
                        t  = new_t;
                        sid = new_sid;
                        ad  = new_ad;
                        found = 1;
...
DataType expression(int *struct_id, int *array_depth) {
    ...
    if (accept(TK_QUESTION)) {
        ...
        int d1, d2; DataType t_true = expression(&d1, &d2);
...
void statement() {
    ...
    if (accept(TK_PRINT)) {
        ...
        if (cur.kind == TK_COMMA) {
            ...
            expect(TK_RPAREN); expect(TK_SEMI); emit(OP_PRINT_FMT); emit(arg_count); return;
        }
    ...
    if (accept(TK_PRINT_CHAR)) { expect(TK_LPAREN); int d1, d2; expression(&d1, &d2); expect(TK_RPAREN); expect(TK_SEMI); emit(OP_PRINT_CHAR); return; }
    if (accept(TK_FREE)) { expect(TK_LPAREN); int d1, d2; expression(&d1, &d2); expect(TK_RPAREN); expect(TK_SEMI); emit(OP_FREE); return; }
    if (accept(TK_RETURN)) {
        if (cur.kind == TK_SEMI) { expect(TK_SEMI); emit(OP_CONST_INT); emit32(0); emit(OP_RET); emit(0); return; }
        int count = 0;
        do { int d1, d2; expression(&d1, &d2); count++; } while (accept(TK_COMMA));
        expect(TK_SEMI); emit(OP_RET); emit(count); return;
    }
    if (accept(TK_WHILE)) {
    ...
    if (accept(TK_FOR)) {
        expect(TK_LPAREN); int saved_locals = local_count; statement();
        size_t start_cond = code_sz; int d1, d2; expression(&d1, &d2); expect(TK_SEMI);
        ...
        expect(TK_RPAREN); emit(OP_JMP); emit32(start_cond); emit_patch(patch_body, code_sz);
        statement(); emit(OP_JMP); emit32(start_step); emit_patch(patch_end, code_sz);
        int diff = local_count - saved_locals; for(int i=0; i<diff; i++) emit(OP_POP); local_count = saved_locals; return;
    }
    if (accept(TK_IF)) {
        expect(TK_LPAREN); int d1, d2; expression(&d1, &d2); expect(TK_RPAREN);
        emit(OP_JZ); size_t patch_else = code_sz; emit32(0);
        statement(); size_t patch_end = 0;
        if (accept(TK_ELSE)) { emit(OP_JMP); patch_end = code_sz; emit32(0); emit_patch(patch_else, code_sz); statement(); emit_patch(patch_end, code_sz); }
        else { emit_patch(patch_else, code_sz); }
        return;
    }
    if (cur.kind == TK_INT || cur.kind == TK_FLOAT || cur.kind == TK_CHAR || cur.kind == TK_STR_TYPE || cur.kind == TK_ID) {
        int sid = -1; int ad = 0; DataType type = TYPE_VOID; int is_decl = 0;
        ...
        if (is_decl) {
            ...
            if (cur.kind == TK_ID) {
                char *name = strdup(cur.text); next(); expect(TK_ASSIGN); int rhs_sid, rhs_ad; DataType rhs_t = expression(&rhs_sid, &rhs_ad);
                if (type == TYPE_STRUCT && rhs_t == TYPE_STRUCT) {
                    sid = rhs_sid;
                }
                expect(TK_SEMI);
                add_local(name, type, sid, ad); return;
            }
        }
    }
    if (cur.kind == TK_ID) {
        char *name = strdup(cur.text); next();
        if (cur.kind == TK_COMMA) {
            char *names[8]; names[0] = name; int count = 1;
            while (accept(TK_COMMA)) { if (cur.kind != TK_ID) fail("Expected variable name"); names[count++] = strdup(cur.text); next(); }
            expect(TK_ASSIGN); if (cur.kind != TK_ID) fail("Expected function call");
            char *func_name = strdup(cur.text); next(); expect(TK_LPAREN);
            int fid = find_func(func_name); if (fid == -1) fail("Undefined function");
            int args = 0; if (cur.kind != TK_RPAREN) { do { int d1, d2; expression(&d1, &d2); args++; } while (accept(TK_COMMA)); }
            expect(TK_RPAREN); expect(TK_SEMI);
            if (args != funcs[fid].arg_count) fail("Arg count mismatch");
            emit(OP_CALL); emit32(funcs[fid].addr); emit(args);
            for (int i = count - 1; i >= 0; i--) {
                int lid = find_local(names[i]); int gid = find_global(names[i]);
                if (lid != -1) { emit(OP_SET_LOCAL); emit(locals[lid].offset); }
                else if (gid != -1) { emit(OP_SET_GLOBAL); emit(gid); }
                else fail("Unknown variable");
                free(names[i]);
            }
            free(func_name); return;
        }
        if (accept(TK_LPAREN)) {
            int fid = find_func(name); if (fid == -1) fail("Undefined function");
            int args = 0; if (cur.kind != TK_RPAREN) { do { int d1, d2; expression(&d1, &d2); args++; } while (accept(TK_COMMA)); }
            expect(TK_RPAREN); expect(TK_SEMI);
            if (args != funcs[fid].arg_count) fail("Arg count mismatch");
            emit(OP_CALL); emit32(funcs[fid].addr); emit(args);
        ...
        // --- FIX: Initialize these to prevent garbage values ---
        DataType t = TYPE_VOID; int sid = -1; int ad = 0;
        ...
        (void)ad;
        if (accept(TK_INC)) { emit(OP_CONST_INT); emit32(1); emit(OP_ADD); if (lid != -1) { emit(OP_SET_LOCAL); emit(locals[lid].offset); } else { emit(OP_SET_GLOBAL); emit(gid); } expect(TK_SEMI); free(name); return; }
        if (accept(TK_DEC)) { emit(OP_CONST_INT); emit32(1); emit(OP_SUB); if (lid != -1) { emit(OP_SET_LOCAL); emit(locals[lid].offset); } else { emit(OP_SET_GLOBAL); emit(gid); } expect(TK_SEMI); free(name); return; }
        if (accept(TK_PLUS_ASSIGN)) { int d1, d2; expression(&d1, &d2); emit(OP_ADD); if (lid != -1) { emit(OP_SET_LOCAL); emit(locals[lid].offset); } else { emit(OP_SET_GLOBAL); emit(gid); } expect(TK_SEMI); free(name); return; }
        if (accept(TK_MINUS_ASSIGN)) { int d1, d2; expression(&d1, &d2); emit(OP_SUB); if (lid != -1) { emit(OP_SET_LOCAL); emit(locals[lid].offset); } else { emit(OP_SET_GLOBAL); emit(gid); } expect(TK_SEMI); free(name); return; }
        while (1) {
            if (accept(TK_DOT)) {
                if (t != TYPE_STRUCT) fail("Dot on non-struct");
                if (sid < 0 || sid >= struct_count)
                    fail("Invalid struct id during field access");
                if (cur.kind != TK_ID) fail("Expected field name");
                int parent_sid = sid; int field_idx = -1; DataType new_t = TYPE_VOID; int new_sid = -1; int new_ad = 0;
                for (int i=0; i<structs[parent_sid].field_count; i++) {
                    if (!strcmp(structs[parent_sid].fields[i].name, cur.text)) {
                        field_idx = i; new_t = structs[parent_sid].fields[i].type; new_sid = structs[parent_sid].fields[i].struct_id; new_ad = structs[parent_sid].fields[i].array_depth; break;
                    }
                }
                if (field_idx == -1) fail("Unknown field");
                next();
                if (cur.kind == TK_ASSIGN) { next(); int d1, d2; expression(&d1, &d2); expect(TK_SEMI); emit(OP_SET_FIELD); emit(structs[parent_sid].fields[field_idx].offset); free(name); return; }
                else if (cur.kind == TK_PLUS_ASSIGN) { next(); emit(OP_DUP); emit(OP_GET_FIELD); emit(structs[parent_sid].fields[field_idx].offset); int d1, d2; DataType rhs_t = expression(&d1, &d2); if (new_t == TYPE_FLOAT || rhs_t == TYPE_FLOAT) emit(OP_ADD_F); else emit(OP_ADD); expect(TK_SEMI); emit(OP_SET_FIELD); emit(structs[parent_sid].fields[field_idx].offset); free(name); return; }
                else if (cur.kind == TK_MINUS_ASSIGN) { next(); emit(OP_DUP); emit(OP_GET_FIELD); emit(structs[parent_sid].fields[field_idx].offset); int d1, d2; DataType rhs_t = expression(&d1, &d2); if (new_t == TYPE_FLOAT || rhs_t == TYPE_FLOAT) emit(OP_SUB_F); else emit(OP_SUB); expect(TK_SEMI); emit(OP_SET_FIELD); emit(structs[parent_sid].fields[field_idx].offset); free(name); return; }
                emit(OP_GET_FIELD); emit(structs[parent_sid].fields[field_idx].offset); t = new_t; sid = new_sid; ad = new_ad;
            } else if (accept(TK_LBRACKET)) {
...
void parse_struct() {
    ...
    int sid = add_struct(strdup(cur.text)); next(); expect(TK_LBRACE); int offset = 0;
...
void parse_func_tail(DataType *ret_types, int *ret_sids, int *ret_ads, int ret_count, char *name) {
    ...
    for(int i=0; i<ret_count; i++) { funcs[fid].ret_types[i] = ret_types[i]; funcs[fid].ret_struct_ids[i] = ret_sids[i]; funcs[fid].ret_array_depths[i] = ret_ads[i]; }
    ...
    if (cur.kind != TK_RPAREN) {
        do {
            ...
            add_local(strdup(cur.text), at, asid, aad); funcs[fid].arg_count++; next();
        } while (accept(TK_COMMA));
    }
    expect(TK_RPAREN); expect(TK_LBRACE);
    while (cur.kind != TK_RBRACE) statement();
    expect(TK_RBRACE);
    if (ret_types[0] == TYPE_VOID && ret_count == 1) { emit(OP_CONST_INT); emit32(0); emit(OP_RET); emit(1); }
}

void parse_func() {
    DataType ret_types[8]; int ret_sids[8]; int ret_ads[8]; int ret_count = 0;
    if (accept(TK_FUNCTION)) {
        ...
        if (accept(TK_COLON)) {
            if (accept(TK_LPAREN)) {
                do {
                    ...
                    if (cur.kind == TK_ID) { add_local(strdup(cur.text), t, sid, ad); next(); }
                } while (accept(TK_COMMA));
                expect(TK_RPAREN);
            } else {
                DataType t; int sid; int ad; parse_type(&t, &sid, &ad);
        ...
        } else { ret_types[0] = TYPE_VOID; ret_sids[0] = -1; ret_ads[0] = 0; ret_count = 1; }
        funcs[fid].ret_count = ret_count;
        for(int i=0; i<ret_count; i++) { funcs[fid].ret_types[i] = ret_types[i]; funcs[fid].ret_struct_ids[i] = ret_sids[i]; funcs[fid].ret_array_depths[i] = ret_ads[i]; }
        expect(TK_LBRACE); while (cur.kind != TK_RBRACE) statement(); expect(TK_RBRACE);
        if (ret_types[0] == TYPE_VOID && ret_count == 1) { emit(OP_CONST_INT); emit32(0); emit(OP_RET); emit(1); }
        return;
    }
...
void parse_program() {
    ...
    while (cur.kind != TK_EOF) {
        if (cur.kind == TK_STRUCT) { parse_struct(); }
        else if (cur.kind == TK_FUNCTION) { emit(OP_JMP); size_t patch = code_sz; emit32(0); parse_func(); emit_patch(patch, code_sz); }
        else {
            DataType t; int sid; int ad; parse_type(&t, &sid, &ad);
            ...
            if (cur.kind == TK_LPAREN) {
                ...
                DataType ret_types[1] = {t}; int ret_sids[1] = {sid}; int ret_ads[1] = {ad};
                parse_func_tail(ret_types, ret_sids, ret_ads, 1, name);
                emit_patch(patch, code_sz);
            } else {
                add_global(name, t, sid, ad); int gid = global_count - 1;
                if (cur.kind == TK_ASSIGN) { next(); int d1, d2; expression(&d1, &d2); emit(OP_SET_GLOBAL); emit(gid); }
                expect(TK_SEMI);
            }
        }
    }
