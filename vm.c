...
void abyss_eye() {
    ...
    int count = 0;
    size_t total_mem = 0;
    printf("\n" C_GRAY "  ╭" C_RESET C_BOLD C_MAGENTA " 👁️  ABYSS EYE " C_RESET C_GRAY "──────────────────────────────────────────╮" C_RESET "\n");
...
int main(int argc, char **argv) {
    ...
    for(uint32_t i=0; i<str_count; i++) { uint32_t len; fread(&len, 4, 1, f); strs[i] = malloc(len+1); fread(strs[i], 1, len, f); strs[i][len] = 0; }

    fread(&struct_count, 4, 1, f);
    structs = malloc(sizeof(StructMeta) * struct_count);
    for(uint32_t i=0; i<struct_count; i++) { uint32_t len; fread(&len, 4, 1, f); structs[i].name = malloc(len+1); fread(structs[i].name, 1, len, f); structs[i].name[len] = 0; fread(&structs[i].size, 4, 1, f); }
    ...
    while (ip < code_size) {
        ...
        switch(op) {
            ...
            case OP_ADD_F: { int64_t ib = pop(); int64_t ia = pop(); double b, a; memcpy(&b, &ib, 8); memcpy(&a, &ia, 8); double r = a + b; int64_t res; memcpy(&res, &r, 8); push(res); break; }
            case OP_SUB_F: { int64_t ib = pop(); int64_t ia = pop(); double b, a; memcpy(&b, &ib, 8); memcpy(&a, &ia, 8); double r = a - b; int64_t res; memcpy(&res, &r, 8); push(res); break; }
            case OP_MUL_F: { int64_t ib = pop(); int64_t ia = pop(); double b, a; memcpy(&b, &ib, 8); memcpy(&a, &ia, 8); double r = a * b; int64_t res; memcpy(&res, &r, 8); push(res); break; }
            case OP_DIV_F: { int64_t ib = pop(); int64_t ia = pop(); double b, a; memcpy(&b, &ib, 8); memcpy(&a, &ia, 8); double r = a / b; int64_t res; memcpy(&res, &r, 8); push(res); break; }
            case OP_NEG_F: { int64_t ia = pop(); double a; memcpy(&a, &ia, 8); a = -a; int64_t res; memcpy(&res, &a, 8); push(res); break; }
            case OP_LT_F: { int64_t ib = pop(); int64_t ia = pop(); double b, a; memcpy(&b, &ib, 8); memcpy(&a, &ia, 8); push(a < b); break; }
            case OP_GT_F: { int64_t ib = pop(); int64_t ia = pop(); double b, a; memcpy(&b, &ib, 8); memcpy(&a, &ia, 8); push(a > b); break; }
            ...
            case OP_NE: { int64_t b=pop(); stack[sp-1] = (stack[sp-1] != b); break; }
            ...
            case OP_PRINT_F: { int64_t iv = pop(); double v; memcpy(&v, &iv, 8); printf("%.6f\n", v); break; }
            ...
            case OP_PRINT_FMT: {
                ...
                for (int i = 0; fmt[i]; i++) {
                    if (fmt[i] == '%' && fmt[i+1] == '{') {
                        i += 2; char type_buf[16]; int ti = 0;
                        while (fmt[i] && fmt[i] != '}' && ti < 15) type_buf[ti++] = fmt[i++];
                        type_buf[ti] = 0;
                        if (current_arg < argc) {
                            int64_t val = stack[sp - argc + current_arg];
                            if (!strcmp(type_buf, "int") || !strcmp(type_buf, "integer")) printf("%ld", val);
                            else if (!strcmp(type_buf, "float")) { double f; memcpy(&f, &val, 8); printf("%.6f", f); }
                            else if (!strcmp(type_buf, "str") || !strcmp(type_buf, "string")) printf("%s", (char*)val);
                            else if (!strcmp(type_buf, "char")) printf("%c", (char)val);
                            current_arg++;
                        }
                    } else putchar(fmt[i]);
                }
            ...
            case OP_CALL: {
                ...
                uint8_t argc = code[ip++];
                call_stack[csp].ret_addr = ip; call_stack[csp].old_fp = fp; csp++;
                fp = sp - argc; ip = addr; break;
            }
            case OP_RET: {
            ...
            case OP_TRY: {
                uint32_t catch_addr; memcpy(&catch_addr, code+ip, 4); ip+=4;
                if (esp >= EXCEPTION_STACK_SIZE) { fprintf(stderr, "Exception Stack Overflow\n"); exit(1); }
...
cleanup:
    free(code);
    for(uint32_t i=0; i<str_count; i++) { free(strs[i]); } free(strs);
    for(uint32_t i=0; i<struct_count; i++) { free(structs[i].name); } free(structs);
    return 0;
}
